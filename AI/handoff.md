...
- [x] 假資料全面刪除潔淨（大廳歷史紀錄、預設題庫與團隊成員清空連雲）
...
- [x] 獨立系統權限與參數管理後台 (`/admin/settings`)：新增 Admin / Supervisor 雙層級授權、手動補同步 Google Sheets 按鈕
...
- [x] 全站核心參數實時動態連動 (`settingsStore.ts`)：考場時間、題目數量、首頁/大廳規則說明、成績單門檻分數連雲綁定
- [x] 考場非同步加載防閃爍機制 (`isQuestionsLoaded`)：防止進入考場時誤跳「無考題」警告
- [x] 題庫後台「Excel 標準模板」動態生成服務（解決舊靜態檔 404/失效問題）
- [x] 考場離開自動刪除未提交草稿 (`deleteExamFirestore`) & 測試資料清空
- [x] 考場超時手動換題與超時防呆鎖定（歸零時暫停並強烈提示，鎖定「確認作答」按鈕）
- [x] 選擇題/問答題獨立限時設定（選擇 120s / 問答 300s）與首頁/大廳規則說明連動
- [x] 選擇題 ABCD 選項順序固定與無情境預設「不限情境作答」
- [x] 合格分數門檻獨立拆分（綜合 `quizPassThreshold` / 申論 `essayPassThreshold`）與舊考卷動態實時門檻比對修復
