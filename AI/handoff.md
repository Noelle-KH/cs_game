...
**專案階段**：✅ 主管批改取消自動跳下一張、右下角回到頂部按鈕、選擇題原生選項 (A, B, C, D) 查閱顯示完成
**分支**：`main`
**PRD 規範**：v1.2
**最後更新**：2026-08-31
...
### 已完成
- [x] PRD 撰寫與確認（v1.2）
- [x] 主管批改後台 (`/admin/grade`) 查閱選擇題時，新增顯示題目原生選項 (A, B, C, D)，並以高亮邊框標示「✅ 正解」與「❌ 考生選擇」
- [x] 主管批改取消自動選取下一張考卷，改完後自動切換至「已批改」頁籤保持目前進度 (`/admin/grade`)
- [x] 於主管批改頁面右下角新增像素風「⬆️ 頂部」回到頁面最上方懸浮按鈕 (`/admin/grade`)
...
- [x] 修復主管批改時因背景 `onSnapshot` 實時拉取新交卷資料導致正在編輯的評語與分數被重置擦除之 Bug (`/admin/grade`)
