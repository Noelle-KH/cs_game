...
### 已完成
- [x] PRD 撰寫與確認（v1.2）
- [x] 主管批改後台 (`/admin/grade`) 一律隱藏 status 為 `resigned` 的離職考生考卷（包含待批改/已批改/已擱置）
- [x] 管理員後台 (`/admin/settings`) 新增「👥 客服考生帳號狀態與資料管理」專區，支援修訂姓名、標記在職/離職 (resigned) 與設定離職生效月份 (`YYYY-MM`)
- [x] 團隊考核狀況與進度總覽 (`/admin/users`) 整合離職過濾邏輯：若離職生效月份 <= 統計選定月份，自動於該選定月份列表中隱藏離職員工
- [x] 團隊考核總覽個人評分 Modal 增設「📅 篩選月份」下拉選單，可快速搜尋特定月份之成績與指導評語細節 (`/admin/users`)
- [x] 冒險排行榜切分月份與下拉選單篩選機制，預設當月（`/leaderboard`）
- [x] 錯題回顧細化問答題評分標定（💯 滿分/⚠️ 部分得分/❌ 0分），並連動錯題（Wrong）篩選器（`/exam/quiz/[examId]/review`）
- [x] 主管批改後台選取考卷與切換考卷時自動觸發平滑置頂（`scrollToTop()`）（`/admin/grade`）
- [x] 錯題回顧頁面 (`/exam/quiz/[examId]/review`) navbar 新增「📊 歷次成績與錯題記錄」直達導覽按鈕
- [x] 配置登入 `browserSessionPersistence` 與 `sessionStorage` 雙重分頁生命週期防護：關閉頁面分頁或瀏覽器後徹底清除 Auth 與 Session，確保重新進入需重新登入
- [x] 團隊考核狀況 (`/admin/users`) 連動 `lastActiveAt` 時間戳記，精準以日期格式（如 `2026/9/1`）展示客服「最近活動時間」
- [x] 主管批改後台 (`/admin/grade`) 評語與指導建議輸入區域擴大（預設 5 行 / min-height 120px），提升閱卷與書寫體驗
- [x] 主管批改後台 (`/admin/grade`) 修正選擇題得分硬編碼分母 (`/ 45 分`)，改為動態精確展示實際得分 (`choiceScore`)
- [x] 主管批改後台 (`/admin/grade`) 全面連動系統動態門檻，並改為直接以 `totalScore >= threshold` 計算狀態，徹底解決交卷殘留舊 `passed: false` 欄位導致高分考卷誤顯示「未通過」之 Bug
- [x] 團隊考核狀況與進度總覽 (`/admin/users`) 限制考卷必須 `status === 'graded'`（主管已完成批改）才納入「綜合最高分」與「申論最高分」計算，避免待批改狀態之中間選擇題得分誤當成最終分數展示
- [x] 新增 `viewer` (觀察員) 帳號身分權限，支援於管理員後台 (`/admin/settings`) 切換，可正常體驗作答，同時自動於「團隊考核狀況 (`/admin/users`)」與「冒險排行榜 (`/leaderboard`)」中排除隱藏
- [x] 管理員後台 (`/admin/settings`) 「👥 客服考生帳號狀態與資料管理」區塊空間擴大（最大高度擴為 `520px`，條目與標籤排版優化），大幅提升閱覽與審視舒適度
...
### 2026-09-02 | 觀察員 Viewer 身分擴充、管理員帳號列表空間擴大、團隊考核與排行榜雙重排除及系統判分邏輯優化

**負責人**：AI  
**開發時長**：約 0.7 小時

#### ✅ 今日完成
1. **新增跨部門觀察員 `viewer` 身分權限與管理功能 (`/admin/settings`)**：
   - 擴充 `UserRole` 型別支援 `'viewer'`。
   - 管理員可在後台「👥 客服考生帳號狀態與資料管理」專區，自由將跨部門體驗帳號切換為 **`👁️ 觀察員 Viewer`**。
   - 該身分帳號可正常參加測驗作答並查看個人歷次成績，但會**自動於「團隊考核狀況總覽 (`/admin/users`)」與「冒險排行榜 (`/leaderboard`)」中排除隱藏**，僅保留正式『考生』登榜，避免影響正式評比與激勵排名。
2. **管理員後台「客服考生帳號狀態與資料管理」區塊空間擴增 (`/admin/settings`)**：
   - 將名單區域最大垂直高度由原本 `240px` 擴增升級至 **`520px`**（增長超過 2.1 倍）。
   - 優化每個成員條目的 Padding、卡片背景及內置計數標籤（顯示當前總成員數），提升閱讀視圖舒適度。
3. **主管批改後台評語與指導建議輸入框體驗優化 (`/admin/grade`)**：
   - 將評語輸入框 HTML `rows` 屬性提升至 `5` 行，並設定 `min-height: 120px`。
4. **選擇題得分顯示與動態門檻判定修正 (`/admin/grade`)**：
   - 移除硬編碼 `/ 45 分`，改為動態顯示。
   - 修正為直接以 `totalScore >= threshold` 比對動態合格門檻。
5. **團隊考核狀況總覽最高分計算過濾 (`/admin/users`)**：
   - 限定考卷必須 `status === 'graded'` 才納入最高分計算。
6. 全面通過 TypeScript `npx tsc --noEmit` 型別驗證。

---

### 2026-09-01 | Session 級別登入持久化、最近活動時間精準連動、錯題回顧直達歷次成績按鈕與個人成績月份切分開發

**負責人**：AI  
**開發時長**：約 3.0 小時

#### ✅ 今日完成
1. **登入 Session 級別持久化機制與關閉頁面自動登出 (`AuthContext.tsx`)**：
   - 引入 Firebase `setPersistence(auth, browserSessionPersistence)` 並配合 `sessionStorage` 進行 Tab Session 生命週期驗證。
   - 確保客服人員或主管在**關閉瀏覽器分頁 (Tab) 或關閉視窗 (Window)** 後，重新進入時絕對強制要求重新登入驗證。
2. **團隊考核「最近活動時間」真實連動精準展示 (`/admin/users`)**：
   - 每次同仁完成登入時，系統自動將當下精確的時間戳記（`lastLoginAt` / `lastActiveAt`）寫回 Firestore `users` 集合。
   - 團隊考核總覽連動讀取該時間戳記，以日期格式展示同仁最近一次實質登入與考核互動時間。
3. **錯題回顧頁面直達「📊 歷次成績與錯題記錄」按鈕 (`/exam/quiz/[examId]/review`)**：
   - 於頂部 Navbar 新增 `📊 歷次成績與錯題記錄` 按鈕，點擊可直接跳轉至個人歷史成績頁 (`/profile/results`)。
4. **個人歷次成績與錯題記錄頁面切分月份 (`/profile/results`)**：
   - Header 新增「📅 統計月份」下拉選單（預設選中 `YYYY-MM (當月)`，亦可選 `🌐 全部歷史紀錄` 及過往歷月）。
   - 切換月份時即時過濾歷史考試清單，並連動統計摘要卡片（總參考場數、通過場數、綜合通過率與最高得分）。
5. **主管批改後台離職考生考卷徹底隱藏 (`/admin/grade`)**：
   - 於 `fetchPendingExams` 中即時讀取 Firestore `users` 集合內標記為 `status === 'resigned'` 的離職人員，一律自動過濾隱藏，避免無效批改。
6. **主管批改後台在職考生與月份下拉篩選器 (`/admin/grade`)**：
   - 增設 **`👤 考生：[ 🌐 全部在職考生 / 具體在職客服 ]`** 下拉選單與「📅 月份選擇」，方便主管快速閱卷。
7. 全面通過 Next.js 生產建置與 `npx tsc --noEmit` 型別驗證。

---

### 2026-08-31 | 主管批改選擇題選項顯示、取消自動跳下一張與「回到頂部」按鈕開發

**負責人**：AI  
**開發時長**：約 0.5 小時

#### ✅ 今日完成
1. **主管批改後台選擇題原生選項 (A, B, C, D) 完整展示 (`/admin/grade`)**：
   - 取消原本批改頁面自動過濾隱藏選擇題的限制，讓綜合模式考卷一併顯示選擇題卡片。
   - 為選擇題卡片新增選項網格卡片，清晰呈現場考卷當下的 4 個原生選項內容 (A, B, C, D)。
   - 以綠色光暈邊框標示 `✅ 正解`，並以紅色邊框標示 `❌ 考生選擇`，方便主管在核對成績或查閱試卷時一目了然題幹與答案選項細節。
2. **取消批改完成後自動跳選下一張考卷 (`/admin/grade`)**：
   - 於 `handleSubmitGrading` 批改送出完成後，將 `setStatusTab('graded')` 並把選中的 `selectedExamId` 清空 (`null`)，同時引入 `isManuallyClearedRef` 機制，確保雲端資料更新時不會強行搶先為主管自動選擇下一張考卷，可讓主管安心查閱剛剛批改好的考卷或再手動選取下一張。
3. **右下角「⬆️ 頂部」一鍵回到頂部懸浮按鈕 (`/admin/grade` & `grade.module.css`)**：
   - 在右側內頁滾動區 (`detailSection`) 綁定滾動事件監聽，當滾動高度超過 200px 時於頁面右下角動態浮現楓之谷像素風格的 `scrollTopBtn` 按鈕。
   - 點擊按鈕能以平滑動畫 (`scrollTo({ top: 0, behavior: 'smooth' })`) 置頂右側詳情區域與瀏覽器主視窗。
4. 全面通過 `npx tsc --noEmit` 型別與編譯檢查。

---

### 2026-08-28 | 主管批改實時更新輸入擦除 Bug 修復

**負責人**：AI  
**開發時長**：約 0.2 小時

#### ✅ 今日完成
1. **主管批改後台實時同步狀態隔離 (`/admin/grade`)**：
   - 修正 `useEffect` 依賴陣列，將原本包含 `currentExam` 的監聽調整為僅監聽 `selectedExamId` 變更。
   - 確保在背景收到其他人新提交的考卷實時更新 (`onSnapshot`) 時，僅更新背景選單與列表數量，不重置主管目前選中並正在編輯中的 `gradingScores` 與 `gradingComments` 輸入框。
2. 通過 `npx tsc --noEmit` 型別與編譯檢查。
